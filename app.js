...
function initRegistrationSystem() {
  ...
  // Helper to dynamically update section headers when registered
  const updateRegistrationHeader = (isRegistered) => {
    const title = document.querySelector('#register .section-title');
    const subtitle = document.querySelector('#register .section-subtitle');
    if (title && subtitle) {
      if (isRegistered) {
        title.innerText = 'Your Access Pass';
        subtitle.innerText = "Congratulations! Your founding member slot is secured.";
      } else {
        title.innerText = 'Request Early Access';
        subtitle.innerText = 'Secure your personal assistant node slot and lock in founding member pricing.';
      }
    }
  };
  ...
  if (cachedUser) {
    try {
      ...
      updateRegistrationHeader(true);
    } catch (e) {
      localStorage.removeItem('thoryn_pre_registered_user');
      updateRegistrationHeader(false);
    }
  }

  if (form) {
    // Auto-detect and parse country code prefixes from typed/pasted numbers
        ...
        if (val.startsWith('+')) {
          ...
          for (const opt of options) {
            ...
            if (val.startsWith(prefix)) {
              ...
              phoneInput.value = val.substring(prefix.length).trim();
              break;
            }
          }
        } 
        // 2. Check if input starts with "00" (international prefix)
        else if (val.startsWith('00')) {
          let valWithPlus = '+' + val.substring(2);
          const options = Array.from(countryCodeSelect.options);
          options.sort((a, b) => b.value.length - a.value.length);
          for (const opt of options) {
            const prefix = opt.value;
            if (valWithPlus.startsWith(prefix)) {
              countryCodeSelect.value = prefix;
              phoneInput.value = valWithPlus.substring(prefix.length).trim();
              break;
            }
          }
        }
        // 3. Check if input starts with raw country code (for codes >= 2 digits, e.g. "880", "91")
        // Restrict to longer input length to avoid false matches on local numbers
        else if (val.length > 7 && !val.startsWith('0')) {
          const options = Array.from(countryCodeSelect.options);
          options.sort((a, b) => b.value.length - a.value.length);
          for (const opt of options) {
            const rawPrefix = opt.value.replace('+', '');
            if (rawPrefix.length >= 2 && val.startsWith(rawPrefix)) {
              countryCodeSelect.value = opt.value;
              phoneInput.value = val.substring(rawPrefix.length).trim();
              break;
            }
          }
        }
    ...
    form.addEventListener('submit', (e) => {
      ...
      const countryCodeSelect = document.getElementById('reg-country-code');
      const phoneInput = document.getElementById('reg-phone');
      const hasPcSelect = document.getElementById('reg-has-pc');

      if (!nameInput || !emailInput || !roleSelect || !countryCodeSelect || !phoneInput || !hasPcSelect) return;

      // Final fallback sanitization check on submit
      ...
      if (phoneVal.startsWith('+')) {
        ...
        options.sort((a, b) => b.value.length - a.value.length);
        for (const opt of options) {
          if (phoneVal.startsWith(opt.value)) {
            selectedCountryCode = opt.value;
            phoneVal = phoneVal.substring(opt.value.length).trim();
            break;
          }
        }
      } else if (phoneVal.startsWith('00')) {
        let valWithPlus = '+' + phoneVal.substring(2);
        const options = Array.from(countryCodeSelect.options);
        options.sort((a, b) => b.value.length - a.value.length);
        for (const opt of options) {
          if (valWithPlus.startsWith(opt.value)) {
            selectedCountryCode = opt.value;
            phoneVal = valWithPlus.substring(opt.value.length).trim();
            break;
          }
        }
      } else if (phoneVal.length > 7 && !phoneVal.startsWith('0')) {
        const options = Array.from(countryCodeSelect.options);
        options.sort((a, b) => b.value.length - a.value.length);
        for (const opt of options) {
          const rawPrefix = opt.value.replace('+', '');
          if (rawPrefix.length >= 2 && phoneVal.startsWith(rawPrefix)) {
            selectedCountryCode = opt.value;
            phoneVal = phoneVal.substring(rawPrefix.length).trim();
            break;
          }
        }
      }

      // Update actual DOM elements visually
      ...
      const userData = {
        ...
        role: roleSelect.options[roleSelect.selectedIndex].text.toUpperCase(),
        phone: `${selectedCountryCode} ${phoneVal}`,
        hasPc: hasPcSelect.options[hasPcSelect.selectedIndex].text.toUpperCase(),
        rank: queueRank,
      ...
      const originalText = submitBtnSpan ? submitBtnSpan.innerText : 'Request Access Pass';
      if (submitBtn) {
        submitBtn.disabled = true;
        if (submitBtnSpan) submitBtnSpan.innerText = 'Requesting Pass...';
      }

      // Create hidden submission iframe if not already present
      ...
      // Create hidden submission form if not already present
      let formElement = document.getElementById('hidden-submit-form');
      if (!formElement) {
        formElement = document.createElement('form');
        formElement.id = 'hidden-submit-form';
        formElement.method = 'POST';
        formElement.action = 'https://script.google.com/macros/s/AKfycbwmVJMHYOR83fNgfyFKCnFniaG0meKWKloetgTBDR6L732Pi5O4wrjp5W-GBhXe8JCa/exec';
        formElement.target = 'hidden-submit-iframe';
      ...
      // Submit form programmatically through the hidden iframe (bypasses CORS restrictions)
      formElement.submit();

      // Proceed immediately to save state and render ticket
...
function renderTicket(userData) {
  ...
  const tRole = document.getElementById('ticket-user-role');
  const tPhone = document.getElementById('ticket-user-phone');
  const tPc = document.getElementById('ticket-user-pc');
  ...
  if (tRole) tRole.textContent = userData.role || userData.useCase || 'STUDENT';
  if (tPhone) tPhone.textContent = userData.phone || 'N/A';
  if (tPc) tPc.textContent = userData.hasPc || 'N/A';
